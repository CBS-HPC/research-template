...
def set_options(programming_language, version_control):
    ...
    def select_version(r_env_manager,python_env_manager):
        ...
        if r_env_manager.lower() == "conda":
            r_version = input(
                "Would you like to specify an R version (e.g., '4.4.3', '4.3', or '4') to be installed via Conda? Leave empty for default: "
            ).strip()
        ...
        if python_env_manager.lower() == "conda":
            python_version = input(
                "Would you like to specify a Python version (e.g., '3.9.3', '3.12', or '3') to be installed via Conda? Leave empty for default: "
            ).strip()
    ...
    if version_control in ["Datalad", "DVC"]:
        remote_storage = prompt_user(
            f"Do you want to setup remote storage for your {version_control} repo:",
