...
def create_tests(programming_language, scripts_folder=None):
    ...
    if not scripts_folder:
        raise ValueError(f"No scripts folder configured for language: {programming_language}")

    # If the scripts folder does not exist, raise an error (do NOT create it)
    if not os.path.isdir(scripts_folder):
        raise ValueError(f"Scripts folder does not exist: {os.path.abspath(scripts_folder)}")
...
def main():
    ...
    programming_language = args.language or load_from_env("PROGRAMMING_LANGUAGE", ".cookiecutter")
