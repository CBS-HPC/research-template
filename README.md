...
- ðŸ“ Clear project structure for transparency and consistency  
- ðŸ§¬ Multi-language support: Python, R, Stata, and Matlab  
- ðŸ—ƒï¸ Built-in version control options: Git, Datalad, or DVC  
- ðŸ“¦ Auto-generated scripts for analysis, modeling, and visualization  
- ðŸ” Environment management via Conda (miniforge) or venv (with UV support)  
- â˜ï¸ Backup integration with ERDA, Dropbox, and OneDrive  
- ðŸš€ Remote repository setup for GitHub, GitLab, or Codeberg  
- ðŸ§ª Unit testing support, TDD scaffolds, and CI integration  
- ðŸ§¾ Auto-generated metadata files: `README.md`, `LICENSE.txt`, `CITATION.cff`
- ðŸ“ Ongoing data management & documentation with RDA maDMP (1.2)
- ðŸ§° Easy activation scripts for both Windows and Bash  
- ðŸ“‘ Structured documentation of all code, data, and dependencies  
- ðŸ“„ Includes support for DCAS-compliant replication packages
- ðŸ“¤ Automated dataset publishing to Zenodo sandbox or DeiC Dataverse
...
> ðŸ’¡ Missing tools (e.g. Git) can be automatically downloaded during setup.
...
<details>
<summary><strong>ðŸ“¦ Online (with Git)</strong></summary><br>
...
<details>
<summary><strong>ðŸ“ Offline (Local Installation)</strong></summary><br>
...
### <a id="license"></a>
<details>
<summary><strong>ðŸ”‘ License</strong></summary><br>
...
<details>
<summary><strong>ðŸ§¬ Language & Environment</strong></summary><br>
...
ðŸ“ Based on your selected language, the template will automatically generate example scripts and notebooks for each stage of the workflow â€” see [How it works](#-how-it-works) for details.
...
ðŸ’¡ Conda (miniforge) will be downloaded and installed automatically if it's not already available.
...
<details>
<summary><strong>ðŸ—ƒï¸ Version Control</strong></summary><br>
...
#### ðŸ“ Auto-generated `.gitignore` includes:
...
> ðŸ§¹ These defaults help keep your repository clean, portable, and reproducible.
...
<details>
<summary><strong>ðŸ“¡ Remote Repository Setup</strong></summary><br>
...
> ðŸ›¡ï¸ Your credentials and tokens are securely stored in the `.env` file and never exposed in plain text.

#### ðŸ” Personal Access Tokens and Permissions
...
##### ðŸ”Ž Required Token Scopes by Platform
...
## ðŸ§¾ How It Works: Structure & Scripts
...
### <a id="cli-tools"></a>
<details>
<summary><strong>ðŸ”§ CLI Tools</strong></summary><br>
...
| `repokit-dmp dataset`            | Initializes or registers datasets (e.g., add metadata, sync folders).       |
...
| `repokit-dmp update`          | Creates and updates `dmp.json` with meta-data from `pyproject.toml` or `cookiecutter.json` |
| `repokit-dmp editor`          | Launches a streamlit app to edit the `dmp.json` or publish datasets to Zenodo or Deic Dataverse |
...
After activating your environment (see [ðŸš€ Project Activation](#-project-activation)), run any command directly:
...
### <a id="backup"></a>
<details>
<summary><strong>ðŸ§° <code>repokit-backup</code></strong></summary>
...
Once your environment is activated (see [ðŸš€ Project Activation](#-project-activation)), you can run the following commands from the terminal:
...
<details>
<summary><strong>ðŸ—ƒï¸ <code>repokit-dmp dataset</code></strong></summary>
...
> ðŸ“ This command is automatically run as part of the setup process but can be rerun manually to resync metadata.
...
> ðŸ’¡ Dataset metadata is stored in `dmp.json` using a normalized schema.  
> ðŸ” All dataset remapping logic happens inside the `repokit.rdm.dataset` module.
...
<details>
<summary><strong>ðŸ“¦ <code>update-dependencies</code></strong></summary>
...
- ðŸ“„ Regenerates `requirements.txt` using `pip freeze`
- ðŸ“¦ Ensures missing packages are added to `uv.lock` (if used)
- ðŸ§ª Scans the `setup/` and `./src/` directories for imports and writes dependency lists:
  - `./setup/dependencies.txt`
  - `./src/dependencies.txt` (or `R/`, `stata/`)
- ðŸ“‘ Updates and tags `environment.yml` and `requirements.txt` with platform-specific selectors (via `platform_rules`)
- ðŸ§  Runs `renv` for R, or language-specific setup scripts for MATLAB and Stata

> ðŸ›  The command adapts to your selected programming language as defined in `.cookiecutter`  
> ðŸ” Paths and rules are derived from the `pyproject.toml` and `platform_rules.json` config

#### Example output:

```bash
ðŸ“„ requirements.txt has been created successfully.
...
<details>
<summary><strong>ðŸ“ <code>update-readme</code></strong></summary>
...
- ðŸ“ Project folder structure as a tree diagram
- ðŸ“¦ Software dependencies (from `dependencies.txt`)
- ðŸ“‘ Auto-generated descriptions for core files and scripts
...
> ðŸ§  File and folder annotations are pulled from `file_descriptions.json`  
> ðŸ—‚ï¸ Files ignored by `.treeignore` or `pyproject.toml â†’ treeignore.patterns` are excluded from the directory tree
...
<details>
<summary><strong>ðŸ’¡ <code>code-examples</code></strong></summary>
...
> ðŸ§  Uses templates from: `./setup/repokit/temples/j2/example`  
> ðŸ—‚ï¸ Script locations depend on your selected programming language  
...
<details>
<summary><strong>ðŸ§± <code>reset-templates</code></strong></summary>
...
> ðŸ§© Uses Jinja2 templates stored in `./setup/repokit/temples/j2/code`  
> ðŸ”„ Existing scripts with the same name may be overwritten!
...
### <a id="git-config"></a>
<details>
<summary><strong>ðŸŒ <code>git-config</code></strong></summary>
...
> ðŸ§ª Remote login and repo creation are tested via platform-specific APIs  
> ðŸ“ Pushes both root repo and data repo (if applicable)  
> ðŸ§° Can auto-install `gh` or `glab` if not found locally
...
### <a id="ci-control"></a>
<details>
<summary><strong>âš™ï¸ <code>ci-control</code></strong></summary>
...
<details>
<summary><strong>ðŸšš <code>dcas-migration</code></strong></summary>
...
<details>
<summary><strong>âœï¸ <code>repokit-dmp editor</code></strong></summary><br>
...
### <a id="code-linting"></a>
<details>
<summary><strong>ðŸ§¹ <code>code-linting</code></strong></summary><br>
...
<details>
<summary><strong>ðŸ› ï¸ Script Templates</strong></summary><br>
...
Once your environment is activated (see [ðŸš€ Project Activation](#-project-activation)), you can use the `backup` CLI tool:
...
<details>
<summary><strong>ðŸ“ Directory Structure</strong></summary><br>
...
ðŸ“ File and folder descriptions are stored in `pyproject.toml` under `[tool.research_template.file_descriptions]`. See the [Configuration Files section](#-configuration-files-root-level) for details.
...
> ðŸ” `activate.*` and `deactivate.*` are either PowerShell (`.ps1`) or Bash (`.sh`) scripts, depending on your platform (Windows or macOS/Linux).

#### ðŸ“¦ Project Subdirectories
...
### <a id="dcas"></a>
<details>
<summary><strong>ðŸ“š DCAS Compatibility</strong></summary><br>
...
- ðŸ“‚ Separation of raw, interim, and processed data
- ðŸ“œ Auto-generated licensing and citation metadata (`LICENSE.txt`, `CITATION.cff`)
- ðŸ§ª Scripted environment setup and reproducibility utilities
- ðŸ“„ Optional DCAS template folder with journal-ready content
...
> ðŸ“ Journal-specific requirements may varyâ€”always consult their latest submission guidelines to ensure full compliance.
