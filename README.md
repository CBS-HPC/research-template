...
├── LICENSE.txt               # Combined license for code, data, and documentation
├── CITATION.cff              # Citation metadata for scholarly reference
├── README.md                 # Main project README with usage and documentation
├── pyproject.toml            # Project metadata and configuration
├── activate.* / deactivate.* # Scripts to activate/deactivate the environment (.ps1 or .sh)
├── environment.yml           # Conda environment definition (Python/R)
...
│   ├── .git/                 # Standalone Git repo for dataset versioning
│   ├── .gitlog               # Git commit log for data tracking
│   ├── 00_raw/               # Original, immutable input data
│   ├── 01_interim/           # Intermediate outputs from cleaning steps
│   └── 02_processed/         # Final, analysis-ready data
├── docs/                     # Reports, supplementary documentation, or rendered output
...
│   ├── setup.py              # Registers `setup/` as a Python package
...
└── src (R/stata)/                  # Main scripts for analysis and data workflows
    ├── dependencies.txt            # Script/module-specific dependencies
    ├── get_dependencies.*          # Retrieves and checks required packages
    ├── s00_main.*                  # Orchestrates the full workflow
    ├── s00_workflow.*              # Notebook-style version of the pipeline
    ├── s01_install_dependencies.*  # Installs project-specific packages
    ├── s02_utils.*                 # Reusable helper functions
    ├── s03_data_collection.*       # Data import or generation logic
    ├── s04_preprocessing.*         # Data cleaning and transformation
    ├── s05_modeling.*              # Statistical modeling or ML components
    └── s06_visualization.*         # Plots and output summaries
```
...
└── src (R/stata)/            # Source code for data processing, analysis, and reporting
    ...
    ├── get_dependencies.*          # Retrieves and checks required packages (utilised)
    ...
    ├── s02_utils.*                 # Shared helper functions for reuse across scripts
